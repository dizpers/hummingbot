...
class DecibelPerpetualUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        await ws.connect(ws_url=ws_url, ping_timeout=CONSTANTS.WS_PING_INTERVAL)
    ...
    async def _subscribe_channels(self, websocket_assistant: WSAssistant) -> None:
        ...
        try:
            ...
            self.logger().info(f"Subscribed to private channels for account {account_addr}")
    ...
    async def _ping_loop(self, ws: WSAssistant):
        ...
        while True:
            try:
                ...
                self.logger().warning("Error sending ping to Decibel WebSocket", exc_info=True)
